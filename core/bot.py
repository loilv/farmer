...
class CandlePatternScannerBot:
    ...
    def _handle_multi_kline_order_queue(self):
        ...
        if 180 <= candle_duration <= 230:
            if len(self.position.keys()) >= 6:
                # logging.info(f'Đã đủ vị thế không thể vào lệnh')
                return
            if symbol not in self.position and abs(percentage_change) >= 5.5:
        ...
        if abs(percentage_change) >= 4.5 and 40 <= candle_duration <= 130:
            if len(self.position.keys()) >= 6:
                # logging.info(f'Đã đủ vị thế không thể vào lệnh')
                return

            if percentage_h <= 0.2 or percentage_l <= 0.2:
